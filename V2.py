...
def convertir_prix(val):
    """
    Convertit des formats de prix courants en nombre :
    12,50 €
    1 250,50
    1.250,50 €
    12.50
    ...
    # Supprimer symboles monétaires, espaces insécables et texte évident
    s = s.replace("\xa0", " ")
    s = s.replace("€", "")
    s = s.replace("EUR", "")
    s = s.replace("eur", "")
    ...
    # Plusieurs points : considérer les derniers comme décimales
    elif s.count(".") > 1:
        morceaux = s.split(".")
...
MOTS_PRODUIT_MOYENS = [
    "nom", "item", "ingredient", "ref produit",
    "designation produit", "article libelle"
]

MOTS_PRIX_FORTS = [
    "prix", "prix ht", "prix ttc", "tarif", "tarif ht",
    "prix unitaire", "pu", "p u", "montant", "cout",
    "cout unitaire", "mini", "minimum", "prix mini",
    "tarif mini", "net ht", "net"
]
...
def score_colonne_produit(df, col):
    nom = nettoyer_nom_colonne(col)
    score = 0

    # Nom de colonne
    for mot in MOTS_PRODUIT_FORTS:
        if nom == mot:
            score += 100
        elif mot in nom:
            score += 65

    for mot in MOTS_PRODUIT_MOYENS:
        if mot in nom:
            score += 40

    for mot in MOTS_PRIX_FORTS:
        if mot in nom:
            score -= 80
    ...
    # Une colonne produit contient généralement beaucoup de texte
    valeurs_non_vides = serie[serie != ""]
    if len(valeurs_non_vides):
        ratio_texte = valeurs_non_vides.map(
            lambda x: convertir_prix(x) is None
        ).mean()
        ...
        # Bonus si les valeurs ont une longueur raisonnable
        longueur_moyenne = valeurs_non_vides.str.len().mean()
        if 4 <= longueur_moyenne <= 80:
            score += 10

    return score


def score_colonne_prix(df, col):
    nom = nettoyer_nom_colonne(col)
    score = 0
    ...
    # Un prix alimentaire raisonnable est généralement positif
    ...
    if len(prix_valides):
        ...
        # Eviter de favoriser des colonnes qui ressemblent à des codes
...
def detecter_colonnes(df):
    """Retourne les meilleures colonnes produit/prix avec leurs scores."""
    if df is None or df.empty:
        return None, None, [], []

    scores_prod = sorted(
        [(col, score_colonne_produit(df, col)) for col in df.columns],
        key=lambda x: x[1],
        reverse=True
    )

    scores_prix = sorted(
        [(col, score_colonne_prix(df, col)) for col in df.columns],
        key=lambda x: x[1],
        reverse=True
    )
    ...
    # Le prix doit être différent du produit
    col_prix = None
    for col, score in scores_prix:
        if col != col_prod:
            col_prix = col
            break
...
def preparer_donnees(df_raw, col_prod, col_prix, fournisseur):
    df = pd.DataFrame()
    ...
    df["Prix_Unitaire"] = df_raw[col_prix].map(convertir_prix)
...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        col_prod, col_prix, scores_prod, scores_prix = detecter_colonnes(df_raw)
